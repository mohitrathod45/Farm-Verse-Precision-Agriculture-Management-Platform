...
import { RiAddLine, RiSearchLine, RiFilterLine, RiPlantLine, RiSunLine, RiCalendarLine, RiDeleteBinLine, RiCloseLine } from 'react-icons/ri';
...
const Crops = () => {
  ...
  const validateForm = () => {
    ...
    if (formData.sowingDate && formData.harvestingDate) {
      if (new Date(formData.harvestingDate) < new Date(formData.sowingDate)) {
        newErrors.harvestingDate = 'Harvest Date cannot be before Sowing Date';
      }
    }
    setErrors(newErrors);
    ...
    <>
      ...
      {loading ? (
        <div className="text-center py-16 bg-white rounded-2xl border border-border-light">
          ...
          <p className="text-text-muted text-sm font-semibold">Loading Crops...</p>
        </div>
      ) : filtered.length === 0 ? (
        <div className="bg-white rounded-2xl p-12 text-center border border-border-light mb-8">
          ...
          {search.trim() ? (
            <>
              <h3 className="text-base font-bold text-text-dark mb-1">No matching crops found</h3>
              <p className="text-xs text-text-muted">No results for &ldquo;{search.trim()}&rdquo;. Try a different keyword.</p>
            </>
          ) : (
            <>
              <h3 className="text-base font-bold text-text-dark mb-1">No Crops Available</h3>
              <p className="text-xs text-text-muted">Click &ldquo;Add Crop&rdquo; to register your first crop.</p>
            </>
          )}
        </div>
      ) : (
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-border-light">
                  <th className="pb-3 text-xs font-bold text-text-muted uppercase tracking-wider px-4">Crop Name</th>
                  ...
                  <th className="pb-3 text-xs font-bold text-text-muted uppercase tracking-wider px-4">Sowing Date</th>
                  <th className="pb-3 text-xs font-bold text-text-muted uppercase tracking-wider px-4">Harvest Date</th>
              ...
              <tbody className="divide-y divide-border-light">
                {filtered.map(c => (
                  <tr key={c.cropId} className="hover:bg-bg-light/60 transition-colors">
                    ...
                    <td className="py-3.5 px-4 text-sm text-text-muted">{formatDate(c.harvestingDate)}</td>
                    <td className="py-3.5 px-4 text-right">
                      <span className={`text-xs font-bold px-2.5 py-1 rounded-full ${getStatusColor(c.status)}`}>
                        {c.status || 'Growing'}
                      </span>
                    </td>
                  </tr>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-3xl max-w-md w-full shadow-2xl border border-border-light overflow-hidden transform transition-all duration-300">
            <div className="px-6 py-4 border-b border-border-light flex items-center justify-between">
              <h3 className="text-lg font-extrabold text-text-dark">Add New Crop</h3>
              <button
            ...
            <form onSubmit={handleAddCrop} className="p-6 space-y-4">
              ...
              {/* Crop Name */}
              <div>
                <label className="block text-xs font-bold text-text-muted uppercase tracking-wider mb-2">Crop Name *</label>
                <input
                  type="text"
                  ...
                  className={`w-full px-4 py-2.5 bg-bg-light border rounded-xl text-sm focus:outline-none focus:ring-2 transition-all text-text-dark ${
                    errors.cropName ? 'border-red-500 focus:ring-red-200' : 'border-border-light focus:ring-primary/20 focus:border-primary'
                  }`}
                />
                {errors.cropName && <p className="text-xs text-red-500 font-semibold mt-1">{errors.cropName}</p>}
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-text-muted uppercase tracking-wider mb-2">Season *</label>
                  <select
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-text-muted uppercase tracking-wider mb-2">Sowing Date</label>
                  <input
                    type="date"
                ...
                <div>
                  <label className="block text-xs font-bold text-text-muted uppercase tracking-wider mb-2">Harvest Date</label>
                  <input
                    type="date"
                    value={formData.harvestingDate}
                    onChange={e => {
                      setFormData({ ...formData, harvestingDate: e.target.value });
                      if (errors.harvestingDate) setErrors({ ...errors, harvestingDate: null });
                    }}
                    className={`w-full px-4 py-2.5 bg-bg-light border rounded-xl text-sm focus:outline-none focus:ring-2 transition-all text-text-dark ${
                      errors.harvestingDate ? 'border-red-500 focus:ring-red-200' : 'border-border-light focus:ring-primary/20 focus:border-primary'
                    }`}
                  />
                  {errors.harvestingDate && <p className="text-xs text-red-500 font-semibold mt-1">{errors.harvestingDate}</p>}
                </div>
              </div>
